...
void flip_bit_in_bmap(uint32_t *bmap, uint32_t bit_position, uint32_t offset_block_idx){
    int bmap_block_idx = bit_position / (BLOCKSIZE * 8);
...
int get_free_bmap_idx(uint32_t *bmap, uint32_t len_bmap, uint32_t bmap_offset){
    int idx = -1; 
    // 0 based idx of the required bit from start of bmap
...
int get_inode_from_disk(int inode_idx, int *inode_block_idx, int *inode_block_offset){
    *inode_block_idx = inode_idx / INODE_PER_BLOCK;
    *inode_block_offset = inode_idx % INODE_PER_BLOCK;
...
int format(disk *diskptr){
    ...
    // initialize inodes
    inode *inodes_block = (inode *)malloc(INODE_PER_BLOCK * sizeof(inode));
    for (int i = 0; i < INODE_PER_BLOCK; i++){
        inodes_block[i].valid = 0;
    }

    for (int i = SB.inode_block_idx; i < SB.data_block_idx; i++){
        if (write_block(diskptr, i, inodes_block) == ERR){
            free(inodes_block);
            return ERR;
        }
    }
